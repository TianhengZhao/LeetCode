...
class Solution:
    def getIntersectionNode_ans(self, headA: ListNode, headB: ListNode) -> ListNode:
        """
        如例1
        node1经过  4-1-8-4-5--5-0-1-8（和node2一样）
        node2经过  5-0-1-8-4-5--4-1-8（和node2一样）
        若无公共结点，node1遍历ListA+ListB后为null，同时node2同理为null，退出while
        """
